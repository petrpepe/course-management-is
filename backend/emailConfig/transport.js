...
class Transport {
  ...
  async buildBody(mail) {
    ...
    return {
      sender:
        !util.isUndefined(addresses.from) && fixupAddresses(addresses.from)[0],
      ...
      replyTo: addresses["reply-to"] && fixupAddresses(addresses["reply-to"]),
      ...
      attachment:
        util.isArray(mail.data.attachments) && mail.data.attachments.length > 0
