...
function UserAction() {
  ...
  <Paper elevation={0} sx={{my: 5, mx: "auto", maxWidth: "1000px"}}>
    ...
    <form onSubmit={onSubmit}>
      ...
      <>
        <CustomSelect id="roles" label="Select roles" items={roles.roles.map(r => {return {_id: r._id, title: r.name}})} itemsStatus={roles.status}
        ...
        <CustomSelect id="extraPerms" label="Select extra permissions" items={avaiblePermissions.map(p => {return {_id: p._id, title: p.name}})} 
         itemsStatus={permissions.status} formData={formData} selectedItems={formData.extraPerms} setFormData={setFormData} multiple={true} />
      </>}
