...
export const createProvider = createAsyncThunk("providers/create", async (providerData, thunkAPI) => {
    try {
...
export const updateProvider = createAsyncThunk("providers/update", async (providerData, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
        return await providerService.updateProvider(providerData._id, providerData, token)
    } catch (error) {
        const message = (error.response && error.response.data && 
            error.response.data.message) || error.message || error.toString()
        return thunkAPI.rejectWithValue(message)
