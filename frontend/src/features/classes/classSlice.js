  ...
  async (classData = {}, thunkAPI) => {
    try {
      ...
      return await classService.getClasses(
        classData.ids ? classData.ids : [],
        classData.keyword ? classData.keyword : "",
        token,
      );
    } catch (error) {
      const message =
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateClass.fulfilled, (state, action) => {
        state.status = Status.Success;
        state.classes[
          state.classes.findIndex((obj) => obj._id === action.payload._id)
        ] = action.payload;
      })
      ...
      .addCase(deleteClass.fulfilled, (state, action) => {
        ...
        state.classes = state.classes.filter(
          (classVar) => classVar._id !== action.payload.id,
        );
      })
